...
#pragma omp parallel for default(none)\
                      private(i, j, k, trial, x, y, z, x1, x2, x3, we, w, vh, vs,\
                             us, ut, wt, w_exact, chk)\
                      shared(N, h, stepsz, a, b, c, nj, nk, ni, seed)\
                      reduction(+:n_inside, dx, dy, dz, err, steps, steps_ave)\
                      collapse(3)
  for (i = 1; i <= ni; i++)
  {
    ...
    for (j = 1; j <= nj; j++)
    {
      ...
      { 
        ...
        if (1.0 < chk)  // ostaje zbog continue
        {
          // w_exact = 1.0; // ne mora i ovako se posle menjaju
          // wt = 1.0;
          steps_ave = 0;
