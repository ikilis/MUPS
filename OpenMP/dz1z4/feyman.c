...
#pragma omp parallel default(none)\
            private(trial, x1, x2, x3, chk, ut, dx, dy, vs, us, vh, we, w, dz, steps, i, j, k,\
                steps_ave, wt, w_exact) \
            shared(N, h, a, b, c, stepsz, x, y, z, seed, err, n_inside, nk, nj, ni)
#pragma omp single 
{
  for (i = 1; i <= ni; i++)
  {
    ...
    for (j = 1; j <= nj; j++)
    {
      ...
      for (k = 1; k <= nk; k++)
      {
        ...
        double tmpErr = pow(w_exact - wt, 2);
        #pragma omp atomic
        err = err + tmpErr;
        }
        // printf("  %7.4f  %7.4f  %7.4f  %10.4e  %10.4e  %10.4e  %8d\n",
