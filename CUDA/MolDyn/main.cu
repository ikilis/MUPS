...
int main()
{
  ...
  // samo jedna alokacija i brisanje (posle petlje)
  ...
  for (move = 1; move <= movemx; move++)
  {
    ...
    cudaMemcpy(f_device, f, npart * 3 * sizeof(double), cudaMemcpyHostToDevice);  // mora jer se resetuje na 0

    // brze od jedne niti da je stavi na 0 u glob
