...
int main(int argc, char **argv)
{
  ...
  chunk = (ni+size-1 )/size;

  start = 1+chunk*rank;
  if(rank != MASTER)start++;
  end = (start+chunk)>ni?ni:(start+chunk);
  // printf("Chunk = %d, mojRank= %d, size = %d, ni = %d, start = %d, end = %d \n",
  //  chunk, rank, size, ni, start, end);
  // vidi da l ces ciklicno ili sa cankovima

  for (i = 1+rank; i <= ni; i+=size)
  {
  ...
  double reduced_err;
  int reduced_n_inside;

  MPI_Reduce(&err, &reduced_err, 1, MPI_DOUBLE, MPI_SUM, MASTER, MPI_COMM_WORLD);
  MPI_Reduce(&n_inside, &reduced_n_inside, 1, MPI_INT, MPI_SUM, MASTER, MPI_COMM_WORLD);
