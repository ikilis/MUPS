  ...
  if(rank == MASTER)
{
  ...
  printf(" total no. of steps ................ %6d\n", movemx);
    ...
    */
    fcc(x, npart, mm, a); // menja X
    /*
    *  Initialise velocities and forces (which are zero in fcc positions)
    */
    mxwell(vh, 3 * npart, h, tref); // menja VH
    dfill(3 * npart, 0.0, f, 1);    // menja F, upisuje svuda 0
    /*
    *  Start of md
    */
    printf("\n    i       ke         pe            e         temp   "
  ...
  for (move = 1; move <= movemx; move++)
  {
    ...
    if(rank == MASTER )
    {
      domove(3 * npart, x, vh, f, side);  // menja X i VH, F=0
    }
    // else
    // {
    //   for(int i =0;i<npart*3; i++)f[i]=0;
    // }
    /*
    ...
    if(rank == MASTER)
    {
      ...
      */
      ekin = mkekin(npart, f, vh, hsq2, hsq); // menja F, VH, EKIN

      /*
      *  Average the velocity and temperature scale if desired
      */
      vel = velavg(npart, vh, vaver, h);  // samo VEL
      if (move < istop && fmod(move, irep) == 0)
      {
        sc = sqrt(tref / (tscale * ekin));
        dscal(3 * npart, sc, vh, 1);      // menja VH
        ekin = tref / tscale;
      }
